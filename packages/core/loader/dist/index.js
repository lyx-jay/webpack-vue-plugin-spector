...
module.exports = function(content) {
  function injectDom(domAst, filePath2, source) {
    if (domAst.type !== 1) {
      return source;
    }
    if (domAst.children && domAst.children.length) {
      for (let i = domAst.children.length - 1; i >= 0; i--) {
        ...
        source = injectDom(node, filePath2, source);
      }
    }
    ...
    const newLine = targetLine.slice(0, columnToInject) + ` ${InjectLineName}="${line}" ${InjectColumnName}="${column}" ${InjectPathName}="${filePath2}" ${InjectNodeName}="${nodeName}"` + targetLine.slice(columnToInject);
